...
import com.ymens.broker.api.endpoint.generic.ReadWriteEndpoint;
...
public abstract class DummyRestEndpoint<T extends Resource> implements ReadWriteEndpoint<T> {
	...
	@Path("{id}")
	@TypeHint(value = Response.class)
	public javax.ws.rs.core.Response patch(@TypeHint(Resource.class) T resource, @PathParam("id") String id) {
		Response<T> response = new Response<T>();
		T res = mock();
